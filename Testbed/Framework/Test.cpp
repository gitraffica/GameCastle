...
void DestructionListener::SayGoodbye(b2Joint* joint)
{
	...
	else
	{
		if(joint->GetUserData()) {
			if(((UserData*)joint->GetUserData())->type == UserData::DISTANCEJOINT) delete (DistanceJointUserData*)joint->GetUserData();
		}
...
Test::Test()
{
	...
	Raffica::effectCNT = 0;
	Raffica::groundInited = false;
	Raffica::buildingList.clear();
	Raffica::drawTitleName2 = "";

	b2Vec2 gravity;
	gravity.Set(0.0f, (float)Raffica::normalG);
	m_world = new b2World(gravity);
	m_particleSystem = m_world->CreateParticleSystem(&particleSystemDef);
	m_particleSystem->SetGravityScale(1);
...
Test::~Test()
{
	...
	Raffica::userdataList.clear();
	Raffica::userdataList.shrink_to_fit();
	Raffica::ButtonList.clear();
	Raffica::ButtonList.shrink_to_fit();
	delete m_world;
	m_world = NULL;
}
...
void Test::DrawTitle(int x, int y, char *string, int lineMax) {
	int start = 0;

	int i = start;
	while(string[i] != '\0') i ++;
	if(i > 500) start = i-500;
    m_debugDraw.DrawString(x, y, string + start, lineMax);
}
...
void Test::MouseMaintain(const b2Vec2& p) {
	mouse = p;
}
...
void Test::MouseUp(const b2Vec2& p)
{
	...
	{
		const int times = 3;
		m_mouseJoint->GetBodyB()->ApplyLinearImpulse(-times * Raffica::normalDamping * m_mouseJoint->GetBodyB()->GetMass() * (m_mouseJoint->GetAnchorB() - m_mouseJoint->GetAnchorA()), m_mouseJoint->GetAnchorB(), true);
...
void Test::LaunchBomb(const b2Vec2& position, const b2Vec2& velocity)
{
	...
	m_bomb = Raffica::createBox(position.x, position.y, 3, 30);
...
void Test::Keyboarding() {
	if (curTest->inputing) return;
	if (upPressed)		Raffica::engineWork(0, 1000);
	if (downPressed)	Raffica::engineWork(0, -1000);
	if (leftPressed)	Raffica::engineWork(-1000, 0);
	if (rightPressed)	Raffica::engineWork(1000, 0);
}

void Test::Step2(Settings* settings) {
}
...
void Test::EnterFrame() {}

b2Body* getBodyAtMouse(const b2Vec2& p, Test* test) {
	b2AABB aabb;
	b2Vec2 d, dd;
	d.Set(1, 1);
	dd.Set(Raffica::button_Xoffset, 0);
	aabb.lowerBound = p - d + dd;
	aabb.upperBound = p + d + dd;
	QueryCallback callback(p + dd);
	test->m_world->QueryAABB(&callback, aabb);
	if (callback.m_fixture) {
		b2Body* body = callback.m_fixture->GetBody();
		//cout << "aaa";
		return body;
	...
	aabb.lowerBound = p-d;
	aabb.upperBound = p+d;
	QueryCallback callback2(p);
	test->m_world->QueryAABB(&callback2, aabb);
	if (callback2.m_fixture) {
		b2Body* body = callback2.m_fixture->GetBody();
		return body;
	}
	return NULL;
}
